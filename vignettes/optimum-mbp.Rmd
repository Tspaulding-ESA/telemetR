...
```{r, setup the new detection frame for optimum mbp analysis, echo = TRUE}
# Join the Receiver data to the detection data by the receiver serial number
data <- dplyr::left_join(detects, receivers, 
                       by = c("ReceiverSN"="ReceiverSN"))

# Receivers can be redeployed leading to multiple instances of the same receiver
# serial number at multiple sites. Remove any rows where the detection does not
# fall between the start and end times of the given receiver deployment.
data <- data |> 
  dplyr::rowwise() |> 
  dplyr::filter(dplyr::between(DateTime_Local, 
                               receiver_start, 
                               receiver_end))

data <- dplyr::left_join(data, fish, by = c("Tag_Code" = "Tag_Code"))
head(data)
```
